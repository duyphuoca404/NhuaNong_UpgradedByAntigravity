...
namespace NhuaNong
{
  public partial class FrmMain : XtraForm
  {
    ...
    {
            //MessageBox.Show("Đã vào hàm EnableFunctions_ByUser");
    try
      {
                try
                {
                ...
                {
                    //MessageBox.Show($"Lỗi khi xóa _lstFuncOfUser: {ex.Message}");
                    MessageBox.Show("Lỗi khi lấy danh sách chức năng: " + ex.Message);
                }
                //FrmMain._lstFuncOfUser = ServiceFactories.GetFactory(ConfigManager.TramTronConfig.RunningMode).ListSEC_Function_ByUserID(objUser.UserID) as List<ObjSEC_Function>;
                //MessageBox.Show($"_lstFuncOfUser is null? {(FrmMain._lstFuncOfUser == null ? "YES" : "NO")}, count: {(FrmMain._lstFuncOfUser != null ? FrmMain._lstFuncOfUser.Count.ToString() : "0")}");
